...
type Shipment = {
  ...
  note: string | null;
  estimated_delivery_date: string | null; // ISO date
  created_at: string;
  updated_at: string;
};
...
export default function ShipmentsPanel({
  ...
  async function onCreate() {
    ...
    try {
      ...
      await reload();
      // Optionally open the manager for the new shipment:
      setActiveShipmentId((s as any)?.id ?? null);
      setManagerOpen(true);
    } catch (e: any) {
          ...
          {rows.map((s) => (
            <div key={s.id} className="p-3 rounded-xl bg-white/[0.04] border border-white/10">
              <div className="flex flex-wrap items-center gap-3 text-sm">
                <b>#{s.id.slice(0, 6)}</b>
                <span>• Status: <span className="font-medium">{s.status ?? "unknown"}</span></span>
                {s.carrier && <span>• Carrier: {s.carrier}</span>}
                {s.tracking_number && <span>• Tracking: {s.tracking_number}</span>}
                {s.estimated_delivery_date && <span>• ETA: {new Date(s.estimated_delivery_date).toLocaleDateString()}</span>}
                <span className="text-white/60">• {new Date(s.created_at).toLocaleString()}</span>
                {canEdit && (
                  <button
                    className="btn ml-auto"
                    onClick={() => { setActiveShipmentId(s.id); setManagerOpen(true); }}
                  >
                    Manage
                  </button>
                )}
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  {NEXT_STEPS[s.status].map((n) => (
                    <button
                      ...
                      disabled={busy}
                    >
                      Mark {String(n).split("_").join(" ")}
                    </button>
                  ))}
                  <button className="btn px-2 py-1 text-xs" onClick={() => move(s.id, "failed")} disabled={busy}>
                    Mark failed
                  </button>
