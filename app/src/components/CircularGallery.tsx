...
class Title {
  ...
  createMesh() {
    ...
    const textH = this.plane.scale.y * 0.14;
...
class Media {
  ...
  title: any; scale = 1; padding = 2; width = 0; widthTotal = 0; x = 0; speed = 0;
  ...
  constructor({ geometry, gl, image, index, length, renderer, scene, screen, text, viewport, bend, textColor, borderRadius = 0, font }) {
    this.geometry = geometry; this.gl = gl; this.image = image; this.index = index; this.length = length;
  ...
  createShader() {
    ...
    this.program = new Program(this.gl, {
      ...
      `,
      fragment: `
        precision highp float; uniform vec2 uImageSizes; uniform vec2 uPlaneSizes;
        uniform sampler2D tMap; uniform float uBorderRadius; varying vec2 vUv;
        float roundedBoxSDF(vec2 p, vec2 b, float r){ vec2 d = abs(p)-b; return length(max(d,0.)) + min(max(d.x,d.y),0.) - r; }
        void main(){
  ...
  update(scroll: any, direction: 'left'|'right') {
    ...
    if (this.bend === 0) { this.plane.position.y = 0; this.plane.rotation.z = 0; }
      ...
      const B = Math.abs(this.bend); const R = (H*H + B*B)/(2*B); const ex = Math.min(Math.abs(x), H);
...
class App {
  container: HTMLDivElement; renderer: any; gl: any; camera: any; scene: any; planeGeometry: any;
  medias: Media[] = []; mediasImages: any[] = []; screen: any; viewport: any;
  scroll: any; scrollSpeed: number; raf = 0; isDown = false; start = 0; onCheckDebounce: any;

  constructor(container: HTMLDivElement, { items, bend, textColor = '#ffffff', borderRadius = 0.05, font = 'bold 30px Figtree', scrollSpeed = 2, scrollEase = 0.05 } = {} as any) {
  ...
  createMedias(items: any[], bend = 1, textColor: string, borderRadius: number, font: string) {
    const defaults = [
      ...
      { image: 'https://picsum.photos/seed/2/800/600', text: 'Sample' },
      { image: 'https://picsum.photos/seed/3/800/600', text: 'Sample' },
    ];
    const galleryItems = items && items.length ? items : defaults;
    this.mediasImages = galleryItems.concat(galleryItems); // seamless loop
  ...
  onTouchMove(e: any) { if (!this.isDown) return; const x = e.touches ? e.touches[0].clientX : e.clientX; const d = (this.start - x) * (this.scrollSpeed * 0.025); this.scroll.target = this.scroll.position + d; }
...
export default function CircularGallery({
  ...
  useEffect(() => {
    ...
    try {
      app = new App(ref.current, { items, bend, textColor, borderRadius, font, scrollSpeed, scrollEase });
    } catch {
      // WebGL not available – silently ignore (the container just stays empty)
    }
    return () => { app?.destroy(); };
