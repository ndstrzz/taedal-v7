...
/* ---------------- wallet artist-pass (stub) ---------------- */
/**
 * This doesn’t generate a real Apple / Google Wallet file yet.
 * It just returns a URL like:
 *   http://localhost:5173/wallet/artist-pass/dev?profile=googled
 * so the client can redirect there.
 */
app.post("/wallet/artist-pass", async (req, res) => {
  try {
    const { uid, userClient } = await getUserAndClients(
      req.headers.authorization
    );
    const { profile_id } = req.body || {};
    if (!profile_id) return res.status(400).send("profile_id required");

    // Optional: verify that the profile belongs to the caller
...
app.post("/pin-artwork", async (req, res) => {
  try {
    ...
    const attrs: Array<{ trait_type: string; value: string | number | boolean }> =
      [];
    if ((art as any).medium)
      attrs.push({ trait_type: "Medium", value: (art as any).medium });
    if ((art as any).year_created)
      attrs.push({ trait_type: "Year", value: (art as any).year_created });
    if ((art as any).width && (art as any).height && (art as any).dim_unit) {
      const dims = (art as any).depth
        ? `${(art as any).width} × ${(art as any).height} × ${
            (art as any).depth
          } ${(art as any).dim_unit}`
        : `${(art as any).width} × ${(art as any).height} ${
            (art as any).dim_unit
...
app.post("/record-mint", async (req, res) => {
  try {
    ...
    if (!artwork_id || !contract_address || !tx_hash) {
      return res
        .status(400)
        .send("artwork_id, contract_address and tx_hash are required");
    }
    ...
    if (art.creator_id !== callerId)
      return res.status(403).send("Only creator can record mint");
...
app.get("/shipments", async (req, res) => {
  try {
    ...
    const artworkId =
      (req.query.artwork_id as string) ||
      (req.query.artworkId as string) ||
      "";
...
app.post("/shipments", async (req, res) => {
  try {
    const { uid, serverClient } = await getUserAndClients(
      req.headers.authorization
    );
    const body = req.body || {};
    const payload = {
    ...
    try {
      ...
      if (art?.creator_id)
        await serverClient
          .from("notifications")
          .insert({ user_id: art.creator_id, ...note });
      if (art?.owner_id && art.owner_id !== art.creator_id)
        await serverClient
          .from("notifications")
          .insert({ user_id: art.owner_id, ...note });
    } catch {
      // ignore
    }
...
app.get("/me/notifications", async (req, res) => {
  try {
    ...
    if (since && !isNaN(since.getTime()))
      q = q.gt("created_at", since.toISOString());
...
app.post("/qr/register", async (req, res) => {
  try {
    const { uid, userClient, serverClient } = await getUserAndClients(
      req.headers.authorization
    );
    const { artwork_id } = req.body || {};
    ...
    const { data: art } = await userClient
      ...
      .maybeSingle();
    if (!art) return res.status(404).send("Artwork not found");
    if ((art as any).creator_id !== uid)
      return res.status(403).send("Only creator can register QR");
...
app.get("/qr/verify", async (req, res) => {
  try {
    ...
    if (!artworkId || !code)
      return res.status(400).send("artworkId & code required");
    ...
    const ok =
      !!(art as any).qr_secret_hash &&
      sha256Hex(code) === (art as any).qr_secret_hash &&
      (art as any).tag_status !== "revoked";
    ...
    res.json({
      result,
      owner_id: (art as any).owner_id,
      creator_id: (art as any).creator_id,
      tag_status: (art as any).tag_status,
    });
  } catch (e: any) {
    res.status(500).send(e?.message || "Server error");
  }
