...
function PriceMarketChart({
  ...
  const points = useMemo(() => {
    ...
    const filtered = rows
      .filter((r) => String(r.currency ?? "").toUpperCase() === String(cur ?? "").toUpperCase())
      .map((r) => ({ t: new Date(r.ts).getTime(), v: Number(r.amount), kind: r.kind }))
      .filter((p) => isFinite(p.t) && isFinite(p.v))
      .sort((a, b) => a.t - b.t);

    return { currency: cur, pts: filtered };
  }, [rows, preferredCurrency]);

  if (!points.pts.length) {
    return <div className="text-sm text-white/60">No price points yet.</div>;
  }

  if (points.pts.length < 2) {
    const p = points.pts[0];
    return (
      <div className="text-sm text-white/70">
        Only 1 data point: <span className="font-semibold">{fmtCurrency(p.v, points.currency)}</span>
      </div>
    );
  }
  ...
  const xs = points.pts.map((p) => p.t);
  const ys = points.pts.map((p) => p.v);
  ...
  const sy = (v: number) => {
    if (y1 === y0) return H - PAD;
    return H - PAD - ((v - y0) / (y1 - y0)) * (H - PAD * 2);
  };

  const poly = points.pts.map((p) => `${sx(p.t)},${sy(p.v)}`).join(" ");
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between text-[11px] text-white/55">
        <span>{start}</span>
        <span>
          Range: {fmtCurrency(minY, points.currency)} → {fmtCurrency(maxY, points.currency)}
        </span>
        <span>{end}</span>
      </div>
      ...
      <div className="rounded-xl border border-white/10 bg-white/[0.03] p-2 overflow-hidden">
        <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-[180px]">
          ...
          <path
            ...
            fill="none"
          />
          <polyline points={poly} fill="none" stroke="rgba(255,255,255,0.85)" strokeWidth="2" />
          {points.pts.map((p, idx) => (
            <circle
              key={idx}
              cx={sx(p.t)}
              cy={sy(p.v)}
              r={p.kind === "sale" ? 3.5 : 2.5}
              fill={p.kind === "sale" ? "rgba(16,185,129,1)" : "rgba(255,255,255,0.85)"}
            />
          ))}
        </svg>
      </div>
      ...
      <div className="flex items-center gap-2 text-[11px] text-white/55">
        <span className="inline-flex items-center gap-1">
          <span className="inline-block w-2 h-2 rounded-full" style={{ background: "rgba(16,185,129,1)" }} />
          Sales
        </span>
        <span className="inline-flex items-center gap-1">
          <span className="inline-block w-2 h-2 rounded-full" style={{ background: "rgba(255,255,255,0.85)" }} />
          Bids
        </span>
        <span className="ml-auto text-white/45">Currency: {points.currency ?? "—"}</span>
      </div>
    </div>
  );
}
...
export default function ArtworkDetail() {
  ...
  // ✅ smart refresh throttle refs (still “every 1s”, but heavy queries are spaced out)
  const lastHeavyRefreshRef = useRef<number>(0);
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      ...
      try {
        // 1) refresh listing (light)
        if (activeListing?.id) {
          await refreshListingByIdSafe(activeListing.id);
        }

        // 2) refresh artwork owner_id (light) so “Owned by” stays correct
        const { data: a2 } = await supabase.from("artworks").select("id,owner_id").eq("id", id).maybeSingle();
        if (alive && a2?.id) {
          setArt((cur) => (cur ? ({ ...cur, owner_id: (a2 as any).owner_id } as any) : cur));
          if ((a2 as any).owner_id) {
            const { data: o2 } = await supabase.from("profiles").select("id,username,display_name,avatar_url").eq("id", (a2 as any).owner_id).maybeSingle();
            if (alive) setOwner((o2 as any)?.data ?? (o2 as any) ?? null);
          }
        }

        // 3) heavy refresh every 5s (owners/sales/price)
        const now = Date.now();
        if (now - lastHeavyRefreshRef.current > 5000 && art?.id) {
          lastHeavyRefreshRef.current = now;
          await Promise.all([loadOwners(art.id), loadSales(art.id), loadPriceHistory(art.id), loadTopOfferSafe(art.id)]);
          if (isAuction && activeListing?.id) {
            await loadBidHistory(activeListing.id);
          }
        }
      } catch {
  ...
  }, [id, activeListing?.id, art?.id, isAuction]);
                  ...
                  <div className="rounded-xl border border-white/10 divide-y divide-white/10 overflow-hidden">
                    {owners.length === 0 ? (
                      ...
                      owners.map((o) => {
                        ...
                        const isCurrent = !!currentOwnerId && o.profile.id === currentOwnerId;
                        return (
