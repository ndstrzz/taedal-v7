...
/** ---------- types ---------- */
type Artwork = { id: string; title: string | null; image_url: string | null; status?: string | null; created_at?: string; updated_at?: string; };
type Listing = { id: string; artwork_id: string; fixed_price: number | null; sale_currency: string | null; status: string; updated_at: string; artwork_title?: string | null; artwork_image_url?: string | null; };
type Sale = { id: string; artwork_id: string; price: number; currency: string; sold_at: string; artwork_title?: string | null; artwork_image_url?: string | null; };
...
export default function StudioHome() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!alive) return;
        setArtworks(A); setPurchased(P); setListings(L); setSales(S);
      } catch (e: any) { setMsg(e?.message || "Failed to load Studio"); }
  ...
  async function loadMyPurchased(userId: string): Promise<Artwork[]> {
    const { data: owns, error } = await supabase
      .from("ownerships")
      .select("artwork_id, quantity, updated_at")
      .eq("owner_id", userId)
      .gt("quantity", 0)
      .order("updated_at", { ascending: false })
      .limit(48);
  ...
  async function loadMyActiveListings(userId: string): Promise<Listing[]> {
    ...
    return (data ?? []).map((r: any) => ({
      id: r.id, artwork_id: r.artwork_id,
      fixed_price: r.fixed_price, sale_currency: r.sale_currency, status: r.status, updated_at: r.updated_at,
      artwork_title: r.artworks?.title ?? null, artwork_image_url: r.artworks?.image_url ?? null,
    })) as Listing[];
  }
  ...
  async function loadMyRecentSales(userId: string): Promise<Sale[]> {
    ...
    if (error) throw error;
    return (data ?? []).map((r: any) => ({
      id: r.id, artwork_id: r.artwork_id, price: r.price, currency: r.currency, sold_at: r.sold_at,
      artwork_title: r.artworks?.title ?? null, artwork_image_url: r.artworks?.image_url ?? null,
    })) as Sale[];
  }
    ...
    <div className="min-h-[calc(100vh-80px)] bg-black">
      <div className="max-w-6xl mx-auto px-6 py-10 space-y-8">
        ...
        >
          ...
          <div className="divide-y divide-white/10">
            {artworks.map(a => {
              ...
              return (
                <Link key={a.id} to={`/art/${a.id}`} className="grid grid-cols-12 items-center gap-3 px-3 py-3 hover:bg-white/[0.04] rounded-lg">
                  ...
                  <div className="col-span-6 md:col-span-2 text-white/80">{floor != null ? `${floor} ${pickCcy(listings, a.id)}` : "—"}</div>
                  <div className="col-span-3 md:col-span-2 text-white/60">{a.updated_at ? new Date(a.updated_at).toLocaleString() : "—"}</div>
        ...
        >
          ...
          <div className="divide-y divide-white/10">
            {listings.map(l => (
              <Link key={l.id} to={`/art/${l.artwork_id}`} className="grid grid-cols-12 items-center gap-3 px-3 py-3 hover:bg-white/[0.04] rounded-lg">
                <div className="col-span-12 md:col-span-6 flex items-center gap-3 min-w-0">
                  <div className="h-10 w-10 rounded-lg bg-neutral-900 overflow-hidden border border-white/10">
                ...
                <div className="col-span-3 md:col-span-2">
                  <span className={`px-2 py-0.5 rounded-md text-xs ${l.status === "active" ? "bg-emerald-400 text-black" : "bg-white/10 text-white"}`}>{l.status}</span>
                </div>
        ...
        >
          ...
          <div className="divide-y divide-white/10">
            {sales.map(s => (
              <Link key={s.id} to={`/art/${s.artwork_id}`} className="grid grid-cols-12 items-center gap-3 px-3 py-3 hover:bg-white/[0.04] rounded-lg">
                <div className="col-span-12 md:col-span-6 flex items-center gap-3 min-w-0">
                  <div className="h-10 w-10 rounded-lg bg-neutral-900 overflow-hidden border border-white/10">
...
function SectionCard({
  title, right, loading, empty, emptyText, children,
}: { title: string; right?: React.ReactNode; loading?: boolean; empty?: boolean; emptyText?: React.ReactNode; children: React.ReactNode; }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-4">
      ...
      {loading ? (
        <div className="space-y-2 animate-pulse">{Array.from({ length: 4 }).map((_, i) => <div key={i} className="h-10 rounded-md bg-white/10" />)}</div>
      ) : empty ? (
